...
def process_karma(message, conn, cursor, timeout):
    ...
    if len(pos_items) + len(neg_items) + len(neut_items) == 1:
        if not pos_items == []:
            ...
            reply = reply + "I have given karma to {0}{2}, the new score is {1}!".format(pos_items[0],
                                                                                         new_scores_pos[0],
                                                                                         reason_string)
        elif not neg_items == []:
            if not neg_dict[neg_items[0]] == "":
                reason_string = " and recorded your reason"
            reply = reply + "I have removed karma from {0}{2}, the new score is {1}.".format(neg_items[0],
                                                                                             new_scores_neg[0],
                                                                                             reason_string)
        else:
            if not neut_dict[neut_items[0]] == "":
                reason_string = " and recorded your reason"
            reply = reply + "I have left the karma of {0} unchanged{2}, the score remains as {1}.".format(neut_items[0],
                                                                                                          new_scores_neut[
                                                                                                              0],
                                                                                                          reason_string)
        ...
        if not list(set(pos_items) & set(neg_items) & set(neut_items)) == []:
            ...
            if len(neut_items) == 1 and len(pos_items) + len(neg_items) == 0:
                count = sum(1 for y in [neut_dict[neut_items[0]], neg_dict[neut_items[0]], pos_dict[neut_items[0]]] if
                            not y == "")
                if count > 0:
                    reason_string = " and recorded your reason"
                if count > 1:
                    reason_string = reason_string + "s"
                reply = reply + "I have left the karma of {0} unchanged{2}, the score remains as {1}.".format(
                    neut_items[0], new_scores_neut[0], reason_string)
                return reply
            ...
            if len(neg_items) == 1 and len(neut_items) + len(pos_items) == 0:
                ...
                if count > 1:
                    reason_string = reason_string + "s"
                reply = reply + "I have left the karma of {0} unchanged{2}, the score remains as {1}.".format(
                    neg_items[0], new_scores_neg[0], reason_string)
                return reply
            ...
            if len(neut_items) == 1 and len(neg_items) + len(pos_items) == 0:
                ...
                if count > 1:
                    reason_string = reason_string + "s"
                reply = reply + "I have given karma to {0}{2}, the new score is {1}!".format(neut_items[0],
                                                                                             new_scores_neut[0],
                                                                                             reason_string)
                return reply
            ...
            if len(neut_items) == 1 and len(neg_items) + len(pos_items) == 0:
                ...
                if count > 1:
                    reason_string = reason_string + "s"
                reply = reply + "I have removed karma from {0}{2}, the new score is {1}.".format(neut_items[0],
                                                                                                 new_scores_neut[0],
                                                                                                 reason_string)
                return reply
...
def update_from_list(items, items_list, k_score, conn, cursor, uid, timeout, reply):
    ...
    for item, reason in items.items():
        ...
        if rows == []:
            ...
            iden, score, pluses, minuses, neutrals = rows[0][0], rows[0][4] + k_score, rows[0][5] + plus, rows[0][
                6] + minus, rows[0][7] + neutral
